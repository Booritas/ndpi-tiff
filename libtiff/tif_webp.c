                      ...
                      const WebPPicture* const picture)
{
  ...
  if ( (tif->tif_rawcc + (tmsize_t)data_size) > tif->tif_rawdatasize ) {
    ...
    _NDPImemcpy(tif->tif_rawcp, data, data_size);
...
TWebPDecode(TIFF* tif, uint8_t* op, tmsize_t occ, uint16_t s)
{
  ...
  if (status != VP8_STATUS_OK && status != VP8_STATUS_SUSPENDED) {
    ...
    if ((buf != NULL) &&
      ...
      NDPIErrorExt(tif->tif_clientdata, module, "Unable to decode WebP data."); 
      return 0;
    }
  }
}
...
TWebPPreEncode(TIFF* tif, uint16_t s)
{
  ...
  sp->pBuffer = _NDPImalloc(sp->buffer_size);
  if( !sp->pBuffer) {
      NDPIErrorExt(tif->tif_clientdata, module, "Cannot allocate buffer");
      return 0;
  }
  sp->buffer_offset = 0;
...
NDPIInitWebP(TIFF* tif, int scheme)
{
  ...
  */
  if ( !_NDPIMergeFields(tif, TWebPFields, TIFFArrayCount(TWebPFields)) ) {
    NDPIErrorExt(tif->tif_clientdata, module,
  ...
  */
  tif->tif_data = (uint8_t*) _NDPImalloc(sizeof(WebPState));
